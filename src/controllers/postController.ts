  ...
  try {
    ...
    if (query.category) {
      const categoryPosts = await db.select()
        .from(postCategories)
        .where(eq(postCategories.categoryId, query.category));
...
const orderByCol = query.sortBy === 'created_at' ? posts.createdAt :
                        query.sortBy === 'updated_at' ? posts.updatedAt :
                        posts.title;
    ...
    const [postsList, totalCount] = await Promise.all([
      db.select({
        ...
        .limit(query.limit)
        .offset(offset),
      db.select({ count: sql<number>`COUNT(${posts.id})` })
        .from(posts)
        .where(whereClause)
